...
getajax(){
            ...
            //console.log(daydateControl);//排列過的順序
      		const numofdata = daydateControl.length;
...
createCalendar(inputdata){
			        ...
			        for (var j = 1; j <= nDays; j++) {
				        ...
				        if(limMonth<10){ monthnum='0'+limMonth }else{ monthnum=limMonth }
			                ...
			                //---------對日期塞detail 
			         		for(let k = 0; k<numofdata; k++){ 
						         const DD = parseInt(inputdata[k].date.substring(8.9));//全部的天數//				         
						         const MM = parseInt(inputdata[k].date.substring(5.7));//月份
						
						         let ddd; if (DD<10){ddd="0"+DD;}else{ddd=DD;}	
						         let mmm; if (MM<10){mmm="0"+MM;}else{mmm=MM;}						                 						         
						         const YYYY = parseInt(inputdata[k].date.substring(0.4));//年份
						         const rr =	''.concat(YYYY,mmm, ddd);

						       if(cuttectDate===rr){      	
					       		if(inputdata[k].status==='額滿' ||inputdata[k].status==='截止' ||inputdata[k].status==='後補'){
							       html += '<span class="status lg-org">'+inputdata[k].status+'</span>' ;
							       }
							    else{
							       html += '<span class="status">'+inputdata[k].status+'</span>' ;
							        	 };
							       html +='<span class="sell">可賣 : '+inputdata[k].availableVancancy+'</span><span class="group">團位 : '+inputdata[k].totalVacnacy+'</span><span class="price">$'+inputdata[k].price+'</span>'
					       		if(inputdata[k].guaranteed == true){
					       		   html +='<span class="tip">保證出團<span>';
					       		}
					        	}		       		
						    	};
